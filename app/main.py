...
def on_validation_error(request, exc) -> JSONResponse:
    return validation_error("Некорректные параметры запроса")


# Healthcheck docker-compose проверки: контейнер готов, когда отвечает 200.
@api.get("/api/health")
def health():
    return {"status": "ok"}
...
def get_flights(
    ...
    return search_flights(
        conn, origin, destination, departure_date, passengers_count
    )


@api.get("/api/flights/{flight_id}")
...
def post_booking(body: dict = Body(...), conn=Depends(get_db)):
    ...
    booking = create_booking(
        conn, flight, data["contact"], data["passengers"]
    )
...
def get_booking_by_code(
    code: str, lastName: str | None = None, conn=Depends(get_db)
):
...
@api.post("/api/bookings/{code}/cancel")
...
# Любой путь под /api/ — JSON-404, а не index.html. HEAD обязателен: без него
# HEAD /api/... провалился бы в SPA-fallback и отдал бы страницу.
@api.api_route(
    "/api/{path:path}",
...
@fallback.api_route("/{path:path}", methods=["GET", "HEAD"])
def spa(path: str):
